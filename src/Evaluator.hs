...
eval (Lambda x t) (ApplyCtx k u) d r a = eval (substituteTermWithNewTerm u (x, t)) k d (r+1) a
...
eval (Con c ts) (CaseCtx k bs) d r a = case find (\(c',xs,t) -> c==c' && length xs == length ts) bs of
                                          ...
                                          Just (c',xs,t) -> eval (foldl (\y (x,t') -> substituteTermWithNewTerm y (x,t')) t $ zip xs ts) 
                                            k d (r+length ts) (a+1)
...
eval (Let x t u) k d r a = eval (substituteTermWithNewTerm u (x,t)) k d (r+1) a 