...
transform index (term@(Con conName expressions), EmptyCtx) funNamesAccum previousGensAccum funsDefs =
  let firstBranch = (Con' conName, doLTS0Tr)
   ...
   in (funsDefs ++ concat otherBranchesFunsDefs, doLTSManyTr term $ (:) firstBranch otherBranchesLtss)
  ...
  case find (\(conName', expressions', _) -> conName == conName' && length expressions' == length expressions) branches of
    ...
    Just (conName', expressions', term') ->
      let oldTerm = place term k
       ...
       in (fst newTerm ++ funsDefs, doLTS1Tr oldTerm (UnfoldCons' conName) $ snd newTerm)
transform index (term@(Lambda x expr), EmptyCtx) funNamesAccum previousGensAccum funsDefs = let
  ...
  in (fst result ++ funsDefs, doLTS1Tr term (Lambda' x) $ snd result)
...
transform index (e@(Let x e0 e1), k) funNamesAccum previousGensAccum funsDefs =
  let
   ...
   in (firstFunsDefs ++ secondFunsDefs, doLTSManyTr (place e k) [firstBranch, secondBranch])
      ...
      let root = Case term branches
          ...
          otherBranchesFunsDefs =
            concatMap
                          ( \(branchName, args, resultTerm) -> let
                                      resultTerm'' = substituteTermWithNewTerms resultTerm [(x', Con branchName $ map Free args)]
                                      in fst $ transform index (resultTerm'', context) funsNames previousGens funsDefs
                          )
                          branches     
       in do { 
       (funsDefs ++ otherBranchesFunsDefs, doLTSManyTr root $ firstBranch : otherBranches)
       }
...
transform' index t@(LTS lts) (CaseCtx context branches) funsNames previousGens funsDefs =
  let root = Case (getOldTerm lts) branches
   ...
   in (otherBranchesFunsDefs ++ funsDefs, doLTSManyTr root $ firstBranch : otherBranches)
