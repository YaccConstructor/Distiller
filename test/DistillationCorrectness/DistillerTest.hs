...
test_and = let
    ...
    result = Case (Free "x") [("True",[],Con "False" []),("False",[],Con "False" [])]
...
test_or = let
    ...
    result = Case (Free "x") [("True",[],Con "True" []),("False",[],Free "y")]
...
test_implies = let
    ...
    funDef = [("implies",(["x","y"],Case (Free "x") [("True",[],Free "x"),("False",[],Con "True" [])]))]
    result = Case (Free "x") [("True",[],Con "True" []),("False",[],Con "True" [])]
    in return $ testGroup "Distiller" [testCase "Distiller: iff True x" $ distillProg (funTerm, funDef) @?= result]    