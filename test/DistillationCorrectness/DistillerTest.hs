...
test_eqBool = let
    ...
    in return $ testGroup "Distiller" [testCase "Distiller: eqBool x False" $ distillProg (funTerm, funDef) @?= result]
...
test_implies = let
    ...
    in return $ testGroup "Distiller" [testCase "Distiller: implies x True" $ distillProg (funTerm, funDef) @?= result]
...
test_append = let 
    ...
    result = Case (Free "xs") [("Nil",[],Free "ys"),("Cons",["x","xs"],Apply (Apply (Fun "append") (Free "xs")) (Free "ys"))]
